    ...
    open: function (event, ui) {
        ...
        ({
            ...
            colModel: [
                ...
                { key: false, label: 'Code', name: 'bom_no1', width: 80, formatter: bom_popup },
                ...
                { key: false, label: 'Lead Time(EA/h)', name: 'need', editable: true, width: '100px', align: 'right' , formatter: 'integer' },
            ...
            onSelectRow: function (rowid, selected, status, e) {
                ...
                if (row_id != null) {
                    ...
                    $("#savestyle_BOM").click(function () {
                        switch ($('#state_BOM').val()) {
                            case 's': 
                                $('#s_bom_no').val(row_id.bom_no);
                                $('#s_bom_no1').val(row_id.bom_no1);
                                
                                break;
                            case 'c':
                                $('#c_bom_no').val(row_id.bom_no);
                                $('#c_bom_no1').val(row_id.bom_no1);
                                
                                break;
                            case 'm':
                                $('#m_bom_no').val(row_id.bom_no);
                                $('#m_bom_no1').val(row_id.bom_no1);
                                break;
                        }
                        $('.dialog_BOM').dialog('close');                              
                    });
        ...
        $('#searchBtn_BOM_popup').click(function () {
            ...
            $.ajax({
                url: "/DevManagement/searchBom_Mgt",
                type: "get",
                dataType: "json",
                data: {
                    bom_no: bom_no,
...
$(".poupdialog_BOM").click(function () {
    switch ($(this).attr('id'))
    {
        case 's_poupdialog_BOM':
            $('#state_BOM').val('s');
            break;
        case 'c_poupdialog_BOM':
            $('#state_BOM').val('c');
            break;
        case 'm_poupdialog_BOM':
            $('#state_BOM').val('m');
            break;
    }
