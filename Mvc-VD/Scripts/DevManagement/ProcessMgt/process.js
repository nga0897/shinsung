...
function _getinfo(lid) {
    $.get(getinfo + "?id=" + lid, function (data) {
        if (data != null && data != undefined) {
            $.each(data, function (key, item) {
                var html = '';
                html += '<table id="info"><tr><td colspan="10" class="text-center text-bold"> <h4 class="tieu_de">Line Information</h4></td></tr><tr><td>Code no </td><td>' + item.process_no + '</td><td>SS Version</td>    <td>' + item.ssver + '</td><td>Customer Rev</td><td>' + item.cust_rev + '</td><td>CAV</td><td>' + item.cav + '</td><td>Register date </td><td>' + item.reg_dt + '</td> </tr>'
                html += '<tr><td>Mode code</td><td>' + item.md_cd + '</td><td>Part Name</td>   <td>' + item.part_nm + '</td><td>Ver</td> <td>' + item.Ver + '</td><td>Type</td><td>' + item.bom_type + '</td><td></td><td></td></tr>';
                html += '<tr><td>Project Name</td><td>' + item.process_nm + '</td><td>Spec</td><td>' + item.spec + '</td><td>Packing Amount(AE)</td><td>' + item.pack_amt + '</td> <td>TDS no</td><td>' + item.tds_no + '</td><td></td><td></td></tr></table>';
                $("#info").html(html);
            });
        }
    });
